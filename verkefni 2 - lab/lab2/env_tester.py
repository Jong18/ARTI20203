...
def main(argv):
    ...
    parser.add_argument("-s", "--simulate", metavar='N', type=int,
                        help="run an N step simulation of the environment using random actions instead of generating all states")
    ...
    print("Creating environment with %dx%d cells and %d dirt spots. Expecting %d states." % (
    width, height, nb_dirts, expected_nb_states))
    ...
    if timediff == 0:
        timediff = 1
    print(
        "Generating those states took: %.2fs (%.0f states/s)" % (timediff / glomma, (glomma * len(states)) / timediff))
    ...
    print("size of all reachable states: %d bytes (%d bytes/state on average)" % (
    size_of_all_states, int(size_of_all_states / len(states))))
    ...
    print("%d unique hash indizes (%.1f%% index collision)" % (
    nb_unique_hash_indices, 100 * (1 - nb_unique_hash_indices / nb_states)))
